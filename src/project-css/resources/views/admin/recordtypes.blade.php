...
<div class="headingWrapper">
  ...
  <div class="container adminHeading" role="banner">
    <span class="text-center">
      <h1><a href="{{ url('table/schema') }}">Define Uploaded Tables</a></h1>
      <p>Please create the record types that are used in this collection. Usually Correspondence data are included with a readme file which can be used to define the records</p>
    </span>
  </div>
</div>
  ...
  <div class="table-responsive">
    <table id="table" name="table" class="table" role="table">
      ...
      <form class="form-horizontal" role="form" method="POST" action="{{url('table/create/finalize')}}">
        ...
        <tbody>
          @foreach($fltFileList as $key => $table)
            <tr>
              <th>{{ $table }}</th>
